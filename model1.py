...
for clust in sorted(df_selected['cluster'].unique()):
    ...
    age_under_50 = (df_selected.loc[mask, "Age at Diagnosis"] < 50).mean()
    age_over_50 = (df_selected.loc[mask, "Age at Diagnosis"] > 50).mean()
...
# Map descriptive labels to final risk groups
descriptive_to_risk_group = {
...
df_selected["final_risk_group"] = df_selected["custom_risk_group"].map(descriptive_to_risk_group)

# SAVE PATIENT LABELS
df_selected['patient_id'] = df_selected.index
result = df_selected[['patient_id', 'custom_risk_group', 'final_risk_group']]
result.to_csv("C:\\Users\\harik\\Downloads\\patient_custom_risk_labels.csv", index=False)
print(result)

# FEATURE IMPORTANCE - RANDOM FOREST
rf = RandomForestClassifier(n_estimators=100, random_state=42)
rf.fit(X, df_selected["final_risk_group"])
...
plt.title("Top 15 Feature Importances for CRC Risk Groups")
plt.xlabel("Importance Score")
plt.ylabel("Feature")
plt.tight_layout()
plt.savefig("C:\\Users\\harik\\Downloads\\feature_importance_chart.png", dpi=300)
plt.show()

print("\nFeature importance chart saved to: C:\\Users\\harik\\Downloads\\feature_importance_chart.png")
